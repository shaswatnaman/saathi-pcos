...
object OfficeBridge {
    ...
    /**
     * Transfers the PDF report to the laptop AND syncs a plain-text summary to
     * the PC clipboard in one shot. Returns a user-facing status message.
     */
    fun sendHealthReport(
        context: Context,
        pdfPath: String,
        risk: RiskAssessment?,
        symptom: SymptomLog?
    ): String {
        val pdfSent = transferFile(context, pdfPath)
        val summary = buildSummary(risk, symptom)
        val clipSynced = syncClipboard(context, summary)
        return when {
            pdfSent && clipSynced ->
                "Report sent to PC + summary copied to clipboard via iQOO Office"
            pdfSent ->
                "Report transferred to your PC via iQOO Office"
            clipSynced ->
                "Summary copied to PC clipboard via iQOO Office"
            else ->
                "Report saved to Downloads (Office Kit transfer failed)"
        }
    }

    private fun buildSummary(risk: RiskAssessment?, symptom: SymptomLog?): String {
        val date = SimpleDateFormat("dd MMM yyyy", Locale.getDefault()).format(Date())
        val level = risk?.riskLevel ?: "No data"
        val score = risk?.totalScore ?: 0
        val acne  = symptom?.let { "%.0f%%".format(it.acneScore * 100) } ?: "—"
        val voice = symptom?.let { "%.0f%%".format(it.voiceEnergyScore * 100) } ?: "—"
        val fatigue = symptom?.fatigue?.toString() ?: "—"
        return """
Saathi PCOS Risk Summary  |  $date
Risk Level : $level  (Score $score/12)

Signal Breakdown
  Acne Score   : $acne   [exploratory — HSV colour analysis]
  Voice Energy : $voice   [experimental — RMS energy]
  Fatigue      : $fatigue/5   [validated — patient-reported]

⚕ This is a screening aid, not a medical diagnosis.
  Generated by Saathi — iQOO Hackathon 2026, Team leadmilers.
""".trimIndent()
    }
